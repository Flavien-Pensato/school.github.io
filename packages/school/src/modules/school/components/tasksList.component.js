import React, { Component } from 'react';
import PropTypes from 'prop-types';
import _ from 'lodash';

import { uuidv4 } from '../../../utils';

import { addTaskAction, removeTaskAction } from '../school.actions';

export class TasksList extends Component {
	onSubmitAddTask = (e) => {
	  e.preventDefault();

	  if (e.target.name.value) {
	    addTaskAction({
	      _id: uuidv4(),
	      name: e.target.name.value,
	    });

	    e.target.name.value = '';
	  }
	};

	render() {
	  const { tasks } = this.props;

	  return (
  <div className="pa3 pa5-ns">
    <ul className="list pl0 measure center">
      {_.map(tasks, (task, index) => <li className="lh-copy pv3 ba bl-0 bt-0 br-0 b--dotted b--black-30" key={index}>{task.name} <button className="bn fr f6 grow no-underline br-pill ph3 pv2 mb2 dib white bg-light-purple" onClick={removeTaskAction(this, task._id)}>Supprimer</button></li>)}
    </ul>

    <form className="measure center br2-ns ba b--black-10" onSubmit={this.onSubmitAddTask}>
      <fieldset className="cf bn ma0 pa0">
        <div className="cf">
          <label htmlFor="name" className="clip">
            <input name="name" type="text" className="f6 f5-l input-reset bn fl black-80 bg-white pa3 lh-solid w-100 w-75-m w-60-l br2-ns br--left-ns" />
								Tâche
          </label>
          <input className="f6 f5-l button-reset fl pv3 tc bn bg-animate bg-black-70 hover-bg-black white pointer w-100 w-25-m w-40-l br2-ns br--right-ns" type="submit" value="Ajouter un nouvelle tache" />
        </div>
      </fieldset>
    </form>
  </div>
	  );
	}
}

TasksList.propTypes = {
  tasks: PropTypes.arrayOf(PropTypes.object).isRequired,
};
